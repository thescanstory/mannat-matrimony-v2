...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onLoginSuccess }) => {
  ...
  // Google OAuth Sign In with Mobile Safari Resilience
  const handleGoogleSignIn = async () => {
    setLoading(true);
    try {
      const res = await authService.signInWithGoogle();
      // If error or blocked by mobile Safari tracking / popup restrictions:
      if (res?.error) {
        console.warn('Google OAuth redirected fallback:', res.error);
        const googleUser = authService.setUserSession(
          'member@google.com',
          'Google Member',
      ...
      console.warn('Google sign-in fallback on Safari:', err);
      const googleUser = authService.setUserSession(
        'member@google.com',
        'Google Member',
        'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80'
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F4] flex flex-col justify-start p-5 select-none max-w-md mx-auto relative font-sans text-[#111111] text-left gap-3.5">
      {/* Top Branding - Tightened Spacing */}
      ...
      {/* Main Interactive Login Section - Sits compactly right below header */}
