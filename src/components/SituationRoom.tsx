...
export const SituationRoom: React.FC<SituationRoomProps> = ({ onComplete, onBackToFeed, scenarios = [] }) => {
  ...
  // No scenarios configured — show an empty state instead of mock data
  if (!currentScenario) {
    return (
      <div className="min-h-screen bg-[#FBF9F4] flex flex-col items-center justify-center px-6 text-center">
        <Sparkles className="w-10 h-10 text-[#B89552] mb-4" />
        <h2 className="text-lg font-bold text-[#161412] mb-2">No scenarios yet</h2>
        <p className="text-sm text-[#7E776F] mb-6 max-w-xs">
          Behavioral scenarios will appear here once they're added. Check back soon!
        </p>
        <button
          onClick={onBackToFeed}
          className="px-6 py-3 rounded-full bg-[#B89552] text-white text-sm font-bold"
        >
          Back to Feed
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#2D2824] text-white p-4 max-w-md mx-auto flex flex-col justify-between">
      {/* Top Header */}
      <div className="pt-4 pb-2">
        <div className="flex items-center justify-between mb-4">
          <button
            onClick={onBackToFeed}
            className="text-xs text-[#7E776F] hover:text-white px-3 py-1.5 rounded-full bg-[#2D2824] border border-[#2D2824]"
          >
            ← Feed
          </button>
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-bold">
            <Sparkles className="w-3.5 h-3.5" />
                        <span>Situation Room ({currentIndex + 1} of {scenarios.length})</span>
          </div>
        </div>

        {/* Progress Bar */}
        <div className="w-full h-1.5 bg-[#2D2824] rounded-full overflow-hidden">
          <div
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {/* Category Tag */}
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#2D2824] border border-[#B89552]/30 text-xs text-amber-400 font-semibold uppercase tracking-widest">
          ...
          <div>
            ...
            <p className="text-xs text-[#7E776F] leading-relaxed">
                ...
                <button
                  ...
                  className={`w-full p-4 rounded-2xl border text-left transition-all flex items-start gap-3 ${
                    isSelected
                      ...
                      : 'bg-[#2D2824]/90 border-[#2D2824] text-[#5C5248] hover:border-[#2D2824]'
                  }`}
                >
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-[#2D2824] to-[#2D2824]/90 border border-amber-500/20 space-y-3">
            ...
            <p className="text-[11px] text-[#7E776F]">
            ...
            <div className="flex items-center gap-3">
              {!isRecording ? (
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-[#2D2824] border border-amber-500/30 text-amber-300 text-xs font-bold hover:bg-[#2D2824] flex items-center justify-center gap-2"
                >
